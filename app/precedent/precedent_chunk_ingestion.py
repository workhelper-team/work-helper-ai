...
def run_precedent_chunk_ingestion():
    ...
    print(f"[Pipeline] 총 {len(precedents)}건의 판례 데이터를 처리합니다.")
    ...
    for prec in precedents:
        ...
        if not full_text_to_chunk:
            print("\n[Skip] 판례 ID {prec_id}: 텍스트 데이터가 없어 스킵합니다.")
            continue
    
        print(f"\n[Processing] 판례: '{case_name}' (사건번호: {case_number})")
        ...
        upsert_precedent_chunks(records)
        print(f"   - DB 적재 완료!")
